# Jonny — Client Profile

**Coach:** Self-programmed | **Program:** 12-Week Body Recomp | **Start:** March 2026

## Intake Summary

| Field | Details |
|-------|---------|
| Age | Mid-20s |
| Height | 5'9" |
| Current Weight | 197 lbs |
| Goal | Body recomp — under 15% BF, visible abs, fill out, not skinny |
| Timeline | March 1 → End of May / Early June (Summer 2026) |
| Desired Feeling | Lean, strong, muscular — back to last summer's shape but better |
| Experience | Experienced — trains at Domain Athletics, runs coaching business |

## Lifestyle

| Factor | Details |
|--------|---------|
| Job | Coaching at Domain Athletics + Marketing agency |
| Nutrition | Meal preps, tracks macros |
| Background | Years of training, understands programming deeply |

## Gyms

- **Primary:** Domain Athletics, Folsom
- **Sometimes:** 24 Hour Fitness, Folsom
- **Sometimes:** Planet Fitness, Folsom

## Preferences

**Enjoys:**
- Mix of dumbbell compounds and machine work
- Heavy deadlift, rows, DB press, hack squat
- Cable work for isolation
- Military presses / overhead pressing for shoulder growth

**Tolerates:**
- Everything — open to all exercises

**Avoids (injury):**
- Barbell bench press (bad left shoulder)
- Barbell back squat (shoulder mobility limits bar position)
- Weighted dips (shoulder stress)

**Weak Points (self-identified):**
- Shoulders (wants boulder caps)
- Arms (fill out t-shirts)

## Coach Notes

- Experienced lifter, body recomp focus — preserve muscle, strip fat
- **Bad left shoulder** — no barbell bench, no back squat, no weighted dips. DB pressing is fine. Overhead pressing (machine) is fine and helps shoulder growth. Chest-supported rows preferred.
- Keep elbows and tricep tendons healthy — warm up thoroughly on Day 5 before pressing/extension work.
- Frequency > Volume. Hit every major group 2x/week, shoulders and arms 3x.
- Day 5 is dedicated weak point work: shoulders + arms
- Has visible muscle mass at 197 — the physique is there, just needs to be uncovered
- Last summer reference photos show he can get very lean with good training
- Programs must work at Domain Athletics, 24 Hour Fitness, and Planet Fitness

---

## Programming Philosophy

| Muscle Group | Frequency | Why |
|-------------|-----------|-----|
| Quads | 2x/week | Lower A + Lower B |
| Glutes | 2x/week | Squat + Hip Thrust + Lunges |
| Hamstrings | 2x/week | RDL + Deadlift + Leg Curl |
| Chest | 2x/week | Upper Push (DB bench + incline + cable pushdown) |
| Back | 2x/week | Upper Pull (T-bar + pulldown + cable row) |
| Shoulders | 3x/week | Push + Pull (face pull) + Day 5 |
| Triceps | 2x/week | Push (cable pushdown) + Day 5 |
| Biceps | 2x/week | Pull (curls) + Day 5 |
| Calves | 1x/week | Lower A |

---

## Week 1 Program

### Day 1 — Lower Body A (Squat-Focused)

| # | Exercise | Sets | Reps | Rest | Notes |
|---|----------|------|------|------|-------|
| 1 | Hack Squat | 3 | 6–8 | 3 min | No shoulder load, go deep, drive through midfoot |
| 2 | Leg Press | 3 | 10–12 | 2.5 min | Feet mid-platform, full depth, don't lock knees |
| 3 | Barbell RDL | 3 | 8–10 | 2.5 min | Push hips back, bar close to shins, feel the hamstring stretch |
| 4 | Leg Extension | 2 | 12–15 | 2 min | Squeeze at top, 2 sec hold, slow negative |
| 5 | Goblet Squat | 2 | 10–12 | 2 min | Elbows inside knees, upright torso, full depth — works anywhere with a DB |

**Total: 13 sets | ~45 min**

---

### Day 2 — Upper Push (Chest + Shoulders + Triceps)

| # | Exercise | Sets | Reps | Rest | Notes |
|---|----------|------|------|------|-------|
| 1 | DB Bench Press | 3 | 6–8 | 3 min | Shoulder-friendly, deep stretch at bottom, press up and in |
| 2 | Incline DB Press | 3 | 8–10 | 2.5 min | 30° incline, deep stretch at bottom, press up and in |
| 3 | Seated DB Shoulder Press | 3 | 8–10 | 2.5 min | Controlled path, don't lock out, feel the delts |
| 4 | Cable Lateral Raise | 2 | 12–15 | 1.5 min | Behind body start, slight lean, pinky up at top |
| 5 | Cable Tricep Pushdown | 2 | 10–12 | 2 min | Elbows pinned, full extension, squeeze at bottom |

**Total: 13 sets | ~40 min**

---

### Day 3 — Lower Body B (Hinge-Focused)

| # | Exercise | Sets | Reps | Rest | Notes |
|---|----------|------|------|------|-------|
| 1 | Trap Bar Deadlift | 3 | 5–6 | 3 min | Push the floor away, chest up, lock out with glutes |
| 2 | Hip Thrust (barbell) | 3 | 8–10 | 2.5 min | Drive through heels, full extension, 2 sec squeeze |
| 3 | Bulgarian Split Squat (DBs) | 3 | 8–10/side | 2.5 min | Front foot flat, slight forward lean, control descent — quad and glute |
| 4 | Lying Leg Curl | 2 | 10–12 | 2 min | Slow eccentric 3 sec, squeeze at top |
| 5 | Adductor Machine | 2 | 12–15 | 1.5 min | Controlled squeeze, slow release |

**Total: 13 sets | ~45 min**

---

### Day 4 — Upper Pull (Back + Rear Delts + Biceps)

| # | Exercise | Sets | Reps | Rest | Notes |
|---|----------|------|------|------|-------|
| 1 | T-Bar Row (chest-supported) | 3 | 6–8 | 3 min | Chest-supported is easier on shoulder, squeeze blades |
| 2 | Lat Pulldown (wide) | 3 | 8–10 | 2.5 min | Full stretch at top, pull to upper chest |
| 3 | Seated Cable Row (close) | 3 | 10–12 | 2 min | Stretch forward, row to waist, hold 1 sec |
| 4 | Face Pull | 2 | 15–20 | 1.5 min | Pull to forehead, external rotate, rear delts on |
| 5 | Barbell Curl | 2 | 10–12 | 1.5 min | Elbows pinned, full range, no body english |

**Total: 13 sets | ~42 min**

---

### Day 5 — Shoulders + Arms (Weak Point)

| # | Exercise | Sets | Reps | Rest | Notes |
|---|----------|------|------|------|-------|
| 1 | Machine Shoulder Press | 3 | 8–10 | 2.5 min | Guided path is safer for shoulder, full range |
| 2 | Cable Lateral Raise | 3 | 12–15 | 1.5 min | Constant tension, slight lean, slow lowering — builds the cap |
| 3 | Overhead Cable Extension | 3 | 10–12 | 2 min | Face away from cable, stretch the long head, elbows by ears |
| 4 | Incline DB Curl | 3 | 10–12 | 2 min | 45° bench, arms hang straight, full stretch at bottom |

**Total: 12 sets | ~35 min**

---

## Weekly Frequency & Volume

| Muscle Group | Sets/Week | Frequency | Hit On |
|-------------|-----------|-----------|--------|
| Quads | ~12 | 2x/week | Lower A (hack squat + leg press + leg ext + BSS) + Lower B |
| Glutes | ~9 | 2x/week | Lower A (RDL + BSS) + Lower B (hip thrust + lunge) |
| Hamstrings | ~8 | 2x/week | Lower A + Lower B |
| Chest | ~8 | 2x/week | Upper Push + Day 5 (overhead press) |
| Back | ~9 | 2x/week | Upper Pull + indirect |
| Shoulders | ~10 | 3x/week | Push + Pull + Day 5 |
| Triceps | ~7 | 2x/week | Push + Day 5 |
| Biceps | ~5 | 2x/week | Pull + Day 5 |
| **Total** | **~55 sets** | | |

---

## Progression Notes

- **Weeks 1–4:** Dial in working weights, prioritize form under heavy load. If hitting top of rep range on all sets, go up 5 lbs compounds / 2.5 lbs accessories.
- **Weeks 5–8:** Push intensity. Add 1 set to compounds if recovery allows. Day 5 — add a 5th exercise (rear delt fly or hammer curl superset).
- **Weeks 9–12:** Peak phase. Heaviest loads. Maintain accessories. Deload week 12 before summer.

---

*Self-programmed by Jonny Valdez — Domain Athletics, Folsom*
